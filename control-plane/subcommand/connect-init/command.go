...
func (c *Command) init() {
	...
	c.flagSet.StringVar(&c.flagBearerTokenFile, "bearer-token-file", "", "Service name as specified via the pod annotation.")
...
func (c *Command) Run(args []string) int {
	...
	err = backoff.Retry(func() error {
		...
		filter := fmt.Sprintf("Meta[%q] == %q and Meta[%q] == %q and (Service == %q or Service == %q)", connectinject.MetaKeyPodName, c.flagPodName, connectinject.MetaKeyKubeNS, c.flagPodNamespace, c.flagServiceName, fmt.Sprintf("%s-sidecar-proxy", c.flagServiceName))
